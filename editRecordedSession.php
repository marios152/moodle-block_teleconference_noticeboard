...
$PAGE->set_title('Edit Recorded Session');
$PAGE->set_url($CFG->wwwroot.'/blocks/teleconference_noticeboard/editRecordSession.php');
...
$sesId= $get_rec->id;
$recordedSessionForm = new teleconf_edit_recorded_session(NULL, array('sesTitle'=>$sesTitle,'sesUrl'=>$sesUrl,'sesId'=>$sesId));
...
if ($recordedSessionForm->is_cancelled()){ //form cancelled
	...
	$session->id						= $data->recordedSessionId;
	...
	$DB->update_record('teleconf_recorded',$session); 
	navigateToCourse($data->courseid);
}
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add('Edit Recorded Session', new moodle_url('/blocks/teleconference_noticeboard/editRecordedSession.php', array('courseid'=>$course->id, 'recordedSessionId'=>$recordedSessionId)));
echo $OUTPUT->header();
echo "<h1>".get_string('editRecordedInfo', 'block_teleconference_noticeboard')."</h1>";
